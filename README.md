...
- ✅ **Nudge full-screen takeover fix** — replaced sticky overlay approach with a proper full-screen takeover: `window.scrollTo({ top: 0, behavior: 'instant' })` fires when refinement starts; recipe fades to 15% opacity with `pointer-events: none` (not 40% — faintly visible as context, not readable or tappable); `RecipeBriefCard` fills the exact content area via `absolute inset-0 z-10` inside a `position: relative` container (no layout shift, no floating overlay); ✕ Cancel refinement button inside the overlay aborts the in-flight `AbortController` and resets all nudge state, fully restoring the original recipe without navigating away. 9 new tests (828 Jest total across 45 suites)
