...
- ✅ **Nudge buttons** — authenticated users see 5 dynamic nudge buttons (🌶️ Spicier · 🥗 Vegetarian · ⚡ Quicker · 🌍 Different cuisine · 🔄 Surprise me) while the `RecipeBriefCard` is visible; tapping cancels the in-flight generation (`AbortController`), fetches a fresh brief with the nudge direction injected as a prompt override via `/api/recipe-brief` (new `nudge` and `forcedCuisine` params), fades the brief card text during the transition (`isNudging` state, Framer Motion opacity), then immediately starts a new generation; "Different cuisine" opens a spring-animated bottom sheet with all 14 cuisines; buttons dynamically hidden based on user preferences (spicier for `hot` spice tolerance, vegetarian for vegan/vegetarian preset, quicker for quick cook time); `abortedByNewerNudge` flag prevents race condition when a second nudge fires before the first generation completes; `CUISINES` array extracted to `lib/cuisines.ts` (shared between ingredients screen and the cuisine picker); `NudgeType` added to `lib/types.ts`; `_signal?: AbortSignal` on `GenerateRecipeInput` so abort propagates through TanStack mutation without serialisation. Guests never see nudge buttons. 22 new tests (800 Jest total across 44 suites)
