...
| `BETTER_AUTH_SECRET` | Random 32-byte secret used by Better Auth to sign session tokens (generate with `openssl rand -hex 32`) |
...
- **Nudge buttons** — authenticated users see a row of 5 dynamic buttons: 🌶️ Make it spicier · 🥗 Make it vegetarian · ⚡ Make it quicker · 🌍 Different cuisine · 🔄 Surprise me. **During generation:** buttons appear in the `RecipeBriefCard`; tapping cancels the in-flight generation (`AbortController`), fetches a fresh brief with the nudge direction injected, fades the brief card during transition, then immediately starts a new generation. **On a completed recipe:** the same buttons appear below the description; tapping scrolls to the top instantly, locks scroll (`overflow: hidden` on both `html` and `body`), and renders a `fixed inset-0 z-50 bg-background` overlay covering the entire viewport — no recipe content is visible or scrollable behind it; a ✕ Cancel refinement button aborts the in-flight request, unlocks scroll, and fully restores the original recipe; when the new recipe arrives the overlay exits and the recipe fades in at full opacity. Both paths pass `existingRecipe` (title + ingredients) to `/api/recipe-brief` and `/api/generate-recipe` so the refinement stays contextually grounded in what just worked. "Different cuisine" opens a spring-animated bottom sheet with all 14 cuisines. Buttons are hidden based on user preferences. Guests never see nudge buttons
- **Ingredient swap "Use this →"** — the ↔ button on any ingredient in the generated recipe opens Substitutes pre-loaded with that ingredient and the full recipe as context; each substitute card then shows a "Use this →" button that calls `POST /api/recipe-update-ingredient` (Claude Haiku) to rewrite the ingredient name and any affected step text; on success, navigates back to the recipe and shows an amber "Swapped X → Y" banner that auto-dismisses after 4 seconds. Auth-required; only shown when opened from a recipe screen
- **Start fresh** — at the bottom of every generated recipe, a "🔄 Start fresh with a new recipe" link reveals an inline confirmation ("Replace this recipe?" with Keep / Start fresh buttons) before navigating back to the ingredient screen to start clean
...
- Full locale support via `next-intl ^4.13.0`; locale auto-detected from the `Accept-Language` HTTP request header — no language switcher UI, no `/en/` URL prefix changes
- 7 locale files: `en`, `es`, `fr`, `de`, `it`, `zh`, `ja` — mirrors Epicure's 7 training languages exactly
- `app/layout.tsx` marked `export const dynamic = 'force-dynamic'` so Vercel always server-renders the layout per request and reads the correct `Accept-Language` header; `<html lang={locale}>` set from `getLocale()` so screen readers and browser translation tools see the correct language
- Translation scope: all UI chrome — navigation labels, auth forms, allergen settings, diet preset cards, ingredient input, recipe controls, pairings, substitutes, history, saved recipes, discover, safe foods, share sheet, tutorial visual sub-components (`KitchenVisual`, `RecipeVisual`, `SafeFoodsVisual`), onboarding slides, and all error / empty states
- Out of scope (English-only by design): recipe content generated by Claude, Epicure ingredient names, Claude outputs
- Adding an eighth language requires only a new `messages/xx.json` — no code changes
...
- Swap icon (↔) on every ingredient row in the generated recipe screen opens Substitutes pre-loaded with that ingredient and the rest of the recipe as context; when opened this way, each substitute card shows "Use this →" to apply the swap directly back into the recipe via `POST /api/recipe-update-ingredient` (see Recipe Generation — Ingredient swap)
...
### Onboarding Tutorial
A 6-slide introductory slideshow that appears on first launch and is re-launchable from settings.
...
- **Slide 3 — Your Cooking Style**: Spice tolerance chip selector (None / Mild / Medium / Hot) and culinary adventurousness selector (Familiar / Occasional / Adventurous); selections persisted to `fable-users` via `/api/user/profile`
- **Slide 4 — Your Kitchen**: Fridge and Cupboard ingredient card mockup
- **Slide 5 — Recipe Generation**: Generated recipe card with Claude attribution and gradient hero
- **Slide 6 — Safe Foods Mode**: Safe Foods list with green shield and ingredient checklist
...
- Dot indicator (pill-shaped active dot) shows current position across all 6 slides
...
- History tab — all recipes generated by the logged-in user, newest first; loaded from DynamoDB on sign-in via `GET /api/user/history` (90-day TTL window); session-only for guests
...
- API route `/api/insights` cached for 1 hour (Next.js route revalidation); `isSignedIn` included in the React Query cache key — insights cache clears immediately on sign-out so no personalized data is ever shown to a logged-out session
- `trendingRecipeTypes` and `trendingPairings` fall back to the all-time aggregate record when the current weekly record is empty (e.g. after the seeded week rolls over), so those sections remain populated without manual re-seeding
...
- ✅ **Multi-turn brief refinement (nudge buttons)** — authenticated users see 5 dynamic nudge buttons (🌶️ Spicier · 🥗 Vegetarian · ⚡ Quicker · 🌍 Different cuisine · 🔄 Surprise me) while the `RecipeBriefCard` is visible; tapping cancels the in-flight generation (`AbortController`), fetches a fresh brief with the nudge direction injected as a prompt override via `/api/recipe-brief` (new `nudge` and `forcedCuisine` params), updates the brief card in-place with a fade transition (`isNudging` state, Framer Motion opacity), then immediately starts a new generation — the screen never leaves the loading state; "Different cuisine" opens a spring-animated bottom sheet with all 14 cuisines; buttons wrap across rows and are dynamically hidden based on user preferences (spicier hidden for `hot` spice tolerance, vegetarian for vegan/vegetarian preset, quicker for quick cook time); `abortedByNewerNudge` flag in `handleNudge` and `wasAborted` flag in `handleGenerateRecipe`/`handleGenerateFromPairings` prevent race conditions and stop aborted `finally` blocks from clearing `loadingStep`; `CUISINES` array extracted to `lib/cuisines.ts`; `NudgeType` added to `lib/types.ts`; `_signal?: AbortSignal` on `GenerateRecipeInput` so abort propagates through TanStack mutation without serialisation. Guests never see nudge buttons. 22 new tests (800 Jest total across 44 suites)
- ✅ **Recipe screen refinement (nudge on complete · ingredient swap · start fresh)** — three interactions that let authenticated users refine a generated recipe without leaving the screen: (1) **Nudge on completed recipe** — the same 5 nudge buttons appear below the recipe description; tapping dims the recipe to 15% opacity and covers the full content area with a `RecipeBriefCard` via `absolute inset-0` overlay while regenerating (see full-screen takeover fix below); both `/api/recipe-brief` and `/api/generate-recipe` receive `existingRecipe` context (title + ingredients) so the new direction stays grounded in what just worked. (2) **Ingredient swap** — "Use this →" button on substitute cards (only when opened from the recipe screen via the ↔ ingredient button); calls new `POST /api/recipe-update-ingredient` (Claude Haiku) which rewrites the ingredient name and any affected step text; on success navigates back to the recipe and shows an amber "Swapped X → Y" banner that auto-dismisses after 4 s. (3) **Start fresh** — "🔄 Start fresh with a new recipe" link at the recipe bottom reveals an inline confirmation before navigating to the ingredient screen. 19 new tests
- ✅ **Nudge full-screen takeover fix** — replaced sticky overlay approach with a proper full-screen takeover: `window.scrollTo({ top: 0, behavior: 'instant' })` fires when refinement starts; `overflow: hidden` applied to both `html` and `body` locks scroll for the duration; overlay is `fixed inset-0 z-50 bg-background` covering the full viewport (not `absolute` inside the container — that left the full-height page scrollable behind it); `RecipeBriefCard` rendered inside a `max-w-2xl mx-auto` wrapper with consistent padding; ✕ Cancel refinement button aborts the in-flight `AbortController`, unlocks scroll, and resets all nudge state; overlay exits via Framer Motion `AnimatePresence` and the original recipe fades back in at full opacity. 9 new tests (828 Jest total across 45 suites)
- ✅ **Low Histamine dietary preset** — `lowHistamine: boolean` field on `UserPreferences`, persisted to `fable-users`; `lib/high-histamine-ingredients.ts` contains 85+ Epicure-verified ingredient keys (fermented foods, aged cheeses, cured meats, vinegars, high-histamine vegetables/fruits, citrus, chocolate, pickled items, fermented Asian condiments) with `ALCOHOL_INGREDIENT_KEYS` spread for single-source-of-truth alcohol coverage; all keys cross-referenced against Epicure's 1,790 ingredient list before inclusion. Hard filter via `effectiveCustomAllergens` in `fable-context`. Prompt clause injected into recipe generation (both normal and safe-foods paths) and recipe brief. Alcoholic and histamine-trigger beverages filtered from drink pairings. Histamine candidates excluded from substitution results. Preset card with ⚕️ disclaimer in allergen settings and onboarding step 3 — named "Low Histamine" (not "No Histamine") to match clinical terminology. **Fix: kitchen ingredient hard filter** — before building the Claude prompt's `humanAvailable` list, `/api/generate-recipe` now filters `adjustedItems` against `customAllergens`; removes the instruction conflict where Claude was simultaneously told to "use" and "avoid" the same ingredient (applies to all `customAllergens` sources, not just low histamine). **Fix: expanded key list** — full Epicure vocabulary audit added `mozzarella_cheese`, `pasta_sauce`, 7 additional cheeses, 5 additional processed meats, `sriracha`, `hot_sauce`, `sherry_vinegar`, `malt_vinegar`, `natto`, `doenjang`, `gochujang`, `pickled_vegetable`, `pickled_ginger`, `milk_chocolate`, `citrus`. 39 tests (867 Jest total across 47 suites)
...
- ✅ **History persistence across sessions** — `recipeHistory` was in-memory only and cleared on every new session, even though history entries are persisted in `fable-saved-recipes` with a 90-day TTL. Added `GET /api/user/history` (returns `isSaved: false` items), a `useHistory` React Query hook, and a context `useEffect` that hydrates `recipeHistory` from DynamoDB on sign-in — same pattern as saved recipes and collections. History is sorted newest-first by `savedAt`. Guests remain session-only.
...
**Kitchen filter before Claude** — `/api/generate-recipe` filters the `adjustedItems` list against `customAllergens` (which includes items from `alcoholMode`, `lowHistamine`, and explicit user allergens) before building the `humanAvailable` string passed to Claude. Removes the instruction conflict where Claude was simultaneously told to "use" and "avoid" the same ingredient — a correctness bug that only surfaces when customAllergens overlap with kitchen contents.

**Least-privilege Lambda IAM** — all four Lambda execution roles use tightly scoped custom IAM policies (specific DynamoDB table ARNs) instead of the `AmazonDynamoDBFullAccess` managed policy. Single-table access pattern per function minimises blast radius if a function is compromised.

**UK-aware cider filter** — `apple_cider` is included in `ALCOHOL_INGREDIENT_KEYS` because in UK/European markets — where Epicure was trained — "cider" means an alcoholic drink by default. `apple_cider_vinegar` is explicitly excluded; the distinction is documented in the key list. Aligns with how Epicure's training data treats these as distinct ingredients.
...
- **867** passing automated tests across 47 suites (+ 35 Lambda tests) ensuring allergen safety and filter accuracy
