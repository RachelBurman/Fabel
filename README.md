...
- ✅ **Low Histamine dietary preset** — `lowHistamine: boolean` field on `UserPreferences`, persisted to `fable-users`; `lib/high-histamine-ingredients.ts` contains 60+ Epicure-verified ingredient keys (fermented foods, aged cheeses, cured meats, vinegars, high-histamine vegetables/fruits, citrus, chocolate) with `ALCOHOL_INGREDIENT_KEYS` spread for single-source-of-truth alcohol coverage; all keys cross-referenced against Epicure's 1,790 ingredient list before inclusion. Hard filter via `effectiveCustomAllergens` in `fable-context`. Prompt clause injected into recipe generation (both normal and safe-foods paths) and recipe brief. Alcoholic and histamine-trigger beverages filtered from drink pairings. Histamine candidates excluded from substitution results. Preset card with ⚕️ disclaimer in allergen settings and onboarding step 3 — named "Low Histamine" (not "No Histamine") to match clinical terminology. 24 new tests (852 Jest total across 46 suites)
