...
- **Nudge buttons** — while the brief card is visible, authenticated users see a horizontal row of 5 dynamic buttons: 🌶️ Make it spicier · 🥗 Make it vegetarian · ⚡ Make it quicker · 🌍 Different cuisine · 🔄 Surprise me. Tapping a nudge cancels the in-flight generation (`AbortController`), fetches a fresh brief from `/api/recipe-brief` with the nudge injected as a prompt override, fades the brief card text during the transition, then immediately starts a new generation. "Different cuisine" opens a spring-animated bottom sheet with all 14 cuisine chips. Buttons are hidden based on user preferences (spicier hidden for `hot` spice tolerance, vegetarian hidden when vegan/vegetarian preset is active, quicker hidden for quick cook time). Guests never see nudge buttons
